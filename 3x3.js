...
function setBingoCardValues(bingoCard) {
        const boxes = document.querySelectorAll('#board1 .box3x3');
        ...
        for (let i = 0; i < bingoCard.length; i++) {
            ...
            for (let j = 0; j < row.length; j++) {
                const value = row[j];
                const boxIndex = (j * bingoCard[0].length) + i;
                const box = boxes[boxIndex];
                box.textContent = value;
                if (value === '★') {
                    box.className='box_filled3x3'
                }
            }
        }

    const boxes2 = document.querySelectorAll('#board2 .box3x3');
    ...
    const boxes3 = document.querySelectorAll('#board3 .box3x3');
    ...
    const boxes4 = document.querySelectorAll('#board4 .box3x3');
...
function numGenerate3x3(){
    if(!finished){
        let num=randomRange(1,30);
...
function checkNumber(number) {
    ...
    boxes.forEach((box) => {
        if (box.textContent === number.toString()) {
            box.classList.remove('box3x3');
            box.classList.add('box_filled3x3');
            filleds1.push(Number(box.id))
            checkBingo3x3(filleds1,1);
        }
    });
...
function checkBingo3x3(filleds,num) {
    points=0
    let allNumbers=[[1,2,3],[4,5,6],[7,8,9],[1,4,7],[2,5,8,],[3,6,9]];
...
function start3x3(){
    names();
    let=bingoCard1=getBingoCard3x3();
    let=bingoCard2=getBingoCard3x3();
    let=bingoCard3=getBingoCard3x3();
    let=bingoCard4=getBingoCard3x3();
